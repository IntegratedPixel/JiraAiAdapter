...
export class ADFBuilder {
  ...
  /**
   * Process a line for special formatting (code blocks, mentions, etc.)
   */
  private static processLine(line: string): ADFNode[] {
    ...
    if (line.startsWith('```')) {
      return [{
        type: 'codeBlock',
        attrs: {
          language: line.slice(3).trim() || 'plain',
        },
        content: [],
      }];
    }
    ...
    const codeMatches = processedLine.matchAll(/`([^`]+)`/g);
    let lastIndex = 0;
    ...
    for (const match of codeMatches) {
      if (match.index! > lastIndex) {
        nodes.push({
          type: 'text',
          text: processedLine.slice(lastIndex, match.index),
        });
      }
